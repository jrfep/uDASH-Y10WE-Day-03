...
make_speed_vs_resistance_data <- function(n_observations=50) {
  d <- data.frame(
    speed = 10 + c(1:n_observations)*50/n_observations) %>% 
    mutate(
        resistance = 0.6 * speed^2 + rnorm(n = n_observations, sd=20)
    )
  write_delim(d, "speedvresistance.txt")
}

make_speed_vs_resistance_data()
```
...
<!-- ### Exercise 1 -->
<!-- 1. Import wind speed and wind resistance data. -->
<!-- 2. Form a hypothesis and choose a predictor and a response variable. -->
<!-- 2. Plot the two variables to visualize the relationship. -->
<!-- 3. Is this relationship linear?  -->
<!-- 4. If not, transform one of the variables to linearise it by either: -->
<!--     - squaring, -->
...
```{r, eval=F}
SvR <- read.delim("speed_vs_drag.txt",header=T,sep="") %>%
  mutate(speedsquared = speed^2)
scatter.smooth(SvR)
svr.lm <- lm(resistance ~ speed, data = SvR)
s2vr.lm <- lm(resistance ~ speedsquared, data = SvR)
plot(svr.lm)
plot(s2vr.lm)
summary(s2vr.lm)
```
...
```{r, echo=F, eval=F}
newdata <- data.frame(speed=c(12,26,72)) %>% mutate(speedsquared = speed^2)
predict(s2vr.lm, newdata) - c(12,26,72)^2 * 0.6
```
